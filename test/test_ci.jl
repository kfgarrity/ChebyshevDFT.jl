...
@testset "test he CI" begin
    ...
    #href 1s1s , 1s2s+2s1s, 1s2p0+2p01s 2s2s 21-1*211 + 211*21-1 210*210
        ...
        dat_he_NONE_p = ChebyshevDFT.GalerkinDFT.dft(; fill_str = "2 0 0 1.0 1.0", g = gal50,N = 100, M = 400,  Z = 2.0,
                                                     niters = 1, mix = 0.0, exc=:hydrogen, exx=1.0, lmax = 1, conv_thr = 1e-11, lmaxrho = 0, mix_lm = false, orthogonalize=false);

        #ci with hydrogen orbitals
        arrI,arrJ,arrH,h, s_up, s_dn = ChebyshevDFT.CI.construct_ham(dat_he_NONE_p, 2, nmax = 2, dense=true);
